...
<?php 
	if (isset($_POST['submit'])) {
		...
		if ($query->num_rows == 0) {
			$sql = "INSERT INTO item (itemName,categoryId, quantity, supplierId, unitPrice, description) ";
			$sql .= "VALUES ('{$_POST['itemName']}', {$_POST['categeory']}, {$_POST['qty']}, {$_POST['sup']}, {$_POST['unitPrice']}, '{$_POST['description']}');";
 ...
 ?>
	...
	<div class="content">
		...
		<form name="add_item" method="post" action="<?php echo chop($_SERVER['PHP_SELF'], '.php'); ?>">
		    ...
		    <button type="submit" name="submit" class="btn" tabindex="7"> Add item </button>
		  </form>
		</div>
